# Complete Hugging Face Deployment and Testing Script
# Run this script to: Create training job → Monitor → Deploy → Test for overfitting
...
Write-Host "HUGGING FACE MODEL DEPLOYMENT" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$json = @"
{
  ...
  "AlgorithmSpecification": {
    "TrainingImage": "763104351884.dkr.ecr.$REGION.amazonaws.com/huggingface-pytorch-training:2.0.0-transformers4.28.1-gpu-py310-cu118-ubuntu20.04",
...
$modelJson = @"
{
  ...
  "PrimaryContainer": {
    ...
    "Mode": "SingleModel",
    "Environment": {
      "SAGEMAKER_PROGRAM": "inference_hf.py",
      "SAGEMAKER_SUBMIT_DIRECTORY": "s3://$MODELS_BUCKET/sagemaker-scripts/sourcedir.tar.gz",
      "SAGEMAKER_REGION": "$REGION",
...
Start-Sleep -Seconds 10  # Give endpoint a moment to stabilize

# Step 5: Test for Overfitting
Write-Host "`nStep 5: Testing for overfitting..." -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan
...
if ($allPositive) {
    ...
    Write-Host "1. Adjust training parameters (epochs, learning rate, batch size)" -ForegroundColor White
    Write-Host "2. Try different model (bert-base-uncased)" -ForegroundColor White
    Write-Host "3. Modify data balancing strategy" -ForegroundColor White
    Write-Host "4. Re-run this script after changes" -ForegroundColor White
    exit 1
}
else {
    ...
    Write-Host "`nRun: git add -A; git commit -m 'Fix overfitting with Hugging Face model'; git push origin main" -ForegroundColor White
    exit 0
}
